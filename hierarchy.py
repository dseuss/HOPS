...
from libhierarchy import fIntegrator, set_omp_threads, init_random_seed
...
class SpectrumHierarchy(Hierarchy):
    ...
    def calc_trajectory(self, psi0=None, label=None, integrator='rk4',
        ...
        self._psi = fIntegrator.run_trajectory_z0(psi_init, integrator,
                                                  **kwargs)
...
class TransferHierarchy(Hierarchy):
    ...
    def _get_rho_partial(self, realizations, psi0, label, id, omp_threads,
        ...
        for i in xrange(realizations):
            ...
            if time() - t_output > OUTPUT_DELAY:
                ...
                t_output = time()
            psi = fIntegrator.run_trajectory(psi0)
    ...
    def calc(self, realizations, psi0, num_threads=-1, omp_threads=1,
        ...
        This function manly distributes and gathers all calculation over
        multiple MPI-nodes.

        :realizations:
        :psi0:
        :num_threads: Number of threads used on the local machine for
                      calculating independent run_trajectories.
        :omp_threads: Number of threads used for parallel calculation of
