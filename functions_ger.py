...
def gauss(w, P=1./np.sqrt(2*np.pi), s=1., m=0.):
  w = np.expand_dims(w, -1)
  x = (w - m) / s
  return np.sum(P / s * np.exp(-x**2 / 2), axis=-1)
...
def J(w, Pt=None, G=None, W=None, P=None, Er=None, H=None, implementation='PFD'):
  """ Calculates the spectral density J(w) as sum of anti-symmetrized
      Lorentzians following
        ...
        J(w | Pt, G, W) = sum_k L(w | P_k, G_k, W_k) - L(w | P_k, G_k, -W_k)
      ...
      energies 'Er' or the heights 'H' can be used to define J(w).
  ...
  if Er is not None: Pt = 4. * G * np.asarray(Er) / np.pi * (W**2 + G**2)
  if H is not None:
...
def spd2bcf(Pt, G, W, T, poles, residues=1.):
  """ Calculates the parameters for an exponential decomposition of the bath
      correlation function
        ...
        α(t) = ∫ dw J(w) [coth(w / 2T) cos(w t) - i sin(w t)]
      ...
      such that
        ...
        α(t) = sum_j P_j exp[-i W_j t - G_j |t|]

      Arguments:
       'Pt'       --  array of weight factors for Lorentz spectral density
  ...
  P_bcf = np.concatenate((
      np.pi / 8 * Pt / (W * G) * (coth(( W + 1j*G) / 2 * Tinv(T)) - 1),
     -np.pi / 8 * Pt / (W * G) * (coth((-W + 1j*G) / 2 * Tinv(T)) - 1),
      np.pi * 1j * 2 * T * residues * J(2 * T * poles, Pt, G, W) ))
...
def t2t2(t):
  """ Calculates the two-sided time array for a one-sided input time array.
      When the input array 't' has the structure [0, dt .. T], the output
      will have [-T .. -dt, 0 .. T-dt] and is calculated using the formula
      
        np.concatenate((-t[1:][::-1], t[:-1]))
      
      See also: fg.t2w, fg.fourier
  """
  return np.concatenate((-t[1:][::-1], t[:-1]))


def w2t(w, hermitian=True):
  N = np.alen(w)
  if hermitian: N = 2 * (N - 1)  # output of 'hfft' has length 2(N-1), not N
  dw = w[1] - w[0]
  dt = 2 * np.pi / (N * dw)
  return dt * (np.arange(N, dtype=float) - N // 2)
  #return 2. * np.pi * fftshift(fftfreq(N, dw))  # this does the same


def w2w2(w):
  return np.concatenate((-w[1:][::-1], w[:-1]))
...
def fourier(f_t, dt, n=None, hermitian=True, output_w = False, sigma_w=0.):
  """ Calculates the correctly normalized Fourier transform of (complex-valued)
      ...
      upper one of these integrals:
                ...
                ∞                          ∞
      F{f(t)} = ∫ dt f(t) exp(+iwt) = 2 Re ∫ dt f(t) exp(+iwt)
               -∞                          ⁰
                       ∞                            ∞
      F⁻¹{f(w)} = 1/2π ∫ dw f(w) exp(-iwt) = 1/π Re ∫ dw f(w) exp(-iwt)
                      -∞                            ⁰
